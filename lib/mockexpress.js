...
var MockExpress = function (route) {
	...
	return {
		...
		'makeRequest': function (getMap) {
			...
			Object.keys(getMap).forEach(function (key) {
				if (!requestObject.hasOwnProperty(key)) {
					requestObject[key] = getMap[key];
				}
			});
			if (!requestObject.cookies) { requestObject.cookies = []; }
			if (!requestObject.ip) { requestObject.ip = "::1"; }
		...
		'makeResponse': function (callback) {
			...
			defaultResponse = {
				...
				cookie: function (cookie) {
					if (sideEffects.cookie) {
						sideEffects.cookie.push(cookie);
					} else {
						sideEffects.cookie = [cookie];
					}
				},
				setHeader: function (header) {
					if (sideEffects.header) {
						sideEffects.header.push(header);
					} else {
						sideEffects.header = [header];
					}
				},
			};
